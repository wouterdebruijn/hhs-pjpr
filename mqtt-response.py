...
print(f"Publishing message... {time.time()}")
then = time.time()

pub = client.publish("your_topic", message, qos=1)
pub.wait_for_publish()

now = time.time()
print(f"Time elapsed: {(now - then) * 1000}ms")

# times = []

# for i in range(0, 100):
#     print(f"Publishing message... {datetime.now().microsecond}")
#     then = time.time()

#     pub = client.publish("your_topic", message, qos=)
#     pub.wait_for_publish()
